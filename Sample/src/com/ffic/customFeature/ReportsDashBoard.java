 ...
 * <p>
 */
public class ReportsDashBoard extends Plugin {
	...
	Logger logger=Logger.getLogger(ReportsDashBoard.class.getName());
	...
	public ReportsDashBoard() {
		String log4jConfigFile = System.getProperty(Constants.edsLog4jPropertyEnvVariable); //"/com/ibm/icn/custom/ffic/plugin/svc/log4j.properties";
		//InputStream log4jStream = BaseService.class.getResourceAsStream(log4jConfigFile);
		PropertyConfigurator.configure(log4jConfigFile);
		

		//Load the config
		if(Configuration.getLastLoaded() == -1)
			try {
				String jsonConfigFile = System.getProperty(Constants.pluginPropertyEnvVariable);
				FileInputStream fs = new FileInputStream(jsonConfigFile);
				Configuration.loadConfiguration(JSONObject.parse(fs));
				
				//Configuration.loadConfiguration(JSONObject.parse(BaseService.class.getResourceAsStream("/com/ibm/icn/custom/ffic/plugin/svc/config.json")));
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public String getConfigurationDijitClass() {
		return "testFeatureDojo.ConfigurationPane";
	}
	...
	public PluginODAuthenticationService getODAuthenticationService() {
		return getODAuthenticationService();
	}
	...
	/**
	    ...
	    public PluginFeature[] getFeatures() {
			...
			return new PluginFeature[] {new com.ffic.customFeature.ReportsFeature()};
	    }
	...
	public PluginService[] getServices() {
		...
		return new PluginService[] {new com.ffic.customService.FeatureService()};
	}
